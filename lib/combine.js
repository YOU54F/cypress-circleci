...
function combineMochaAwesomeReports() {
  const reportDir = process.cwd('cypress/mochawesome-report');
...
function writeReport(obj, uuid) {
  ...
  const outFile = path.join((process.cwd(),`${uuid}.json`));
...
shell.exec(`./node_modules/.bin/marge ${uuid}.json -i --saveJson --reportDir ~/mochareports --reportTitle ${uuid}`, (code, stdout, stderr) => {
  if (stderr) {
    console.log(stderr);
  } else {
    console.log('Success!');
